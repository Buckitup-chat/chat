...
import { keccak256, zeroPadValue, hexlify, Wallet, SigningKey, toBeHex } from 'ethers';
...
export class SingleKeyPair {
	...
	constructor(privateKey) {
		...
		this.account = new Wallet(privateKey);
		this.account.publicKey = new SigningKey(privateKey).publicKey;
	}
}
...
export class BuckItUpClient {
	...
	generateSharesEncrypted = async (secret, numShares, threshold, publicKeys) => {
		...
		const encryptedShares = unencryptedShares.map((share, index) => {
			...
			return encryptWithPublicKey(publicKey.slice(2), share).then((encrypted) => '0x' + cipher.stringify(encrypted));
		});
	...
	decryptShare = async (encryptedShare, privateKey) => {
		...
		const decrypted = await decryptWithPrivateKey(privateKey.slice(2), encryptedObject);
		return decrypted;
	};
	...
	generateStealthAddress = (metaStealthPublicKey) => {
		...
		const ephemeralPrivateKey = ephemeral.privateKey;
		...
		const sharedSecret = getSharedSecret(ephemeralPrivateKey.slice(2), metaStealthPublicKey.slice(2), false);

		const hashedSharedSecret = keccak256(hexlify(sharedSecret.slice(1)));
	...
	getStealthAddressFromEphemeral = (metaStealthPrivateKey, ephemeralPubKey) => {
		const metaStealthPublicKey = new SigningKey(metaStealthPrivateKey).publicKey;

		//const metaStealth = privateKeyToAccount(metaStealthPrivateKey);
		const sharedSecret = getSharedSecret(metaStealthPrivateKey.slice(2), ephemeralPubKey.slice(2), false);
		const hashedSharedSecret = keccak256(hexlify(sharedSecret.slice(1)));
		const R_pubkey_spend = ProjectivePoint.fromHex(metaStealthPublicKey.slice(2));
		const stealthPublicKey = R_pubkey_spend.multiply(BigInt(hashedSharedSecret));
	...
	generateStealthPrivateKey = (metaStealthPrivateKey, ephemeralPubKey) => {
		const sharedSecret = getSharedSecret(metaStealthPrivateKey.replace('0x', ''), ephemeralPubKey.replace('0x', ''), false);
		...
		return zeroPadValue(toBeHex(privateKeyBigInt), 32);
	};
}
