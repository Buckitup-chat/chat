...
The v2 migration plan ([files/pq_files.done.md §14](files/pq_files.done.md#14-migration-plan-chunk-blobs--filesystem-protocol-v2-raw-bytes)) moves chunk bytes out of PostgreSQL onto the filesystem and introduces `ChunkFetcher` for device-to-device byte transfer. Two problems remain:
...
Same as [files/pq_files.done.md §14.2](files/pq_files.done.md#142-protocol-v2-changes) with `cid` added and `data_b64` removed:
...
Same as [files/pq_files.done.md §1.1](files/pq_files.done.md#11-files-electric-synced). The trust chain is unmodified: `chunk_sign_hashes[i] = SHA3-512(chunk_i.sign_b64)`.

### 3.3 `upload_chunks` (unchanged)

Same as [files/pq_files.done.md §1.3](files/pq_files.done.md#13-upload_chunks-local-only-not-electric-synced). Budget accounting uses `size` from manifest rows — unaffected by IPFS.

### 3.4 `missing_chunks` (local only, NOT Electric-synced)

Same structure as [files/pq_files.done.md §14.3](files/pq_files.done.md#143-target-architecture) with `cid` added:
...
Population follows the same two-stage pre-seed as v2 ([files/pq_files.done.md §14.3](files/pq_files.done.md#143-target-architecture)): `files` manifest arrival creates placeholders → `file_chunks` manifest rows fill `cid`, `data_hash`, `size`.
...
On chunk admission (any source): compute `SHA3-512(raw bytes)`, verify against `data_hash` from the signed manifest. IPFS independently computes SHA2-256 for the CID. The trust chain ([files/pq_files.done.md §14.3](files/pq_files.done.md#143-target-architecture) "Receiver integrity") is unchanged — the bytes channel is untrusted by design; integrity is enforced at admission, not in transport.

## 5. Upload Protocol

Same as [files/pq_files.done.md §14.2](files/pq_files.done.md#142-protocol-v2-changes) chunk upload endpoint, with IPFS replacing filesystem storage:
...
Upload authentication is unchanged from v2: the chunk signature is the proof of possession — no challenge flow ([files/pq_files.done.md §14.2](files/pq_files.done.md#142-protocol-v2-changes) "Upload authentication").
...
The [BootSupervisor](files/pq_files.done.md) staged startup gains an IPFS-related step:
...
BuckitUp GC ([files/pq_files.done.md §8](files/pq_files.done.md#8-garbage-collection)) must coordinate with IPFS:
