...
export class Enigma {
   ...
   */
   signChallenge(challenge, privateKeyB64) {
      ...
      const rBytes = this.bigintToBytes(r); // 32 bytes
      const sBytes = this.bigintToBytes(s); // 32 bytes
    ...
    */
   recoverPublicKey(challenge, signatureCombinedB64) {
      // Hash the input challenge using SHA-256
      const digest = this.base64ToArray(this.base64ToSha256(Buffer.from(challenge).toString("base64")));  

      // Decode the combined base64 signature
      const combined = this.base64ToArray(signatureCombinedB64);  
      ...
      return this.arrayToBase64(publicKey.toRawBytes(true)); 
   }

   bigintToBytes(value, byteLength = 32) {
   ...
   compactPubpikKeyToAddress(publickKeyB64) {
      const pkh = this.convertPublicKeyToHex(publickKeyB64)
   ...
   generateKeypair() {
      ...
      const publicKey = this.getPublicKeyFromPrivateKey(privateKey, true);
