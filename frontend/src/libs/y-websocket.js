 ...
 * @param {WebsocketProvider} provider
 */
const setupWS = async (provider) => {
	if (provider.shouldConnect && provider.ws === null) {
		const ts = Math.floor(Date.now() / 1000);

		const message = `${provider.roomname}:${ts}`;
		const wallet = new Wallet(provider.privateKey);
		const sig = await wallet.signMessage(message);

		const query = url.encodeQueryParams({ sig, ts });
		provider.url = provider.serverUrl + '/' + provider.roomname + '?' + query;
