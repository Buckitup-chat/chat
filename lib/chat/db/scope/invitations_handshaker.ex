defmodule Chat.Db.Scope.InvitationsHandshaker do
  ...
  def put_room_invitations_content(context, snap, keys) do
  ...
  def process_invitation_groups(invitations_queue) do
    invitations_queue
    |> Enum.reduce(
      %{
  ...
  def process_invitation_group(invitation_group, acc) do
    group_data =
      invitation_group
  ...
  def fetch_cycle_invites(sender_keys, %{invitations_messages: invitations_messages} = _context) do
  ...
  def get_messages_sender_keys(messages, move) do
    messages
  ...
  def start_invitations_queue(%{invitations_queue: _invitations_queue} = context, _pub_keys),
    do: context

  def start_invitations_queue(%{invitations_messages: invitations_messages} = context, pub_keys) do
    invitations_messages
  ...
  def put_invitations_queue([], context), do: context

  def put_invitations_queue(invitations, %{invitations_queue: queue} = context),
    do:
      Map.put(
        context,
        :invitations_queue,
  ...
  def put_invitations_queue(invitations, context),
    do: context |> Map.put(:invitations_queue, [invitations])

  def remove_queued_invitations_messages(
  ...
  def clear_context_preparation(%{room_invites: _, room_invite_indexes: _} = context),
    do: Map.drop(context, [:room_invites, :room_invite_indexes, :invitations_keymap])

  def put_invitations_messages(context, snap) do
  ...
  def put_invitations_keymap(context) do
  ...
  def union_set_acc(set_acc, group_data, key_extractor) do
    group_data
    |> Enum.map(&key_extractor.(&1))
    |> union_set(set_acc)
  end

  def update_acc_set(acc_set, consumed_keys) do
    acc_set
  ...
  def extract_invite_key({:room_invite_index, _, invite_key}), do: invite_key

  def filter_invitations_indexes(invitations, _reader_hash_getter, nil), do: invitations

  def filter_invitations_indexes(invitations, reader_hash_getter, keys) do
  ...
  def filter_invitations_records(invitations, nil), do: invitations

  def filter_invitations_records(invitations, keys) do
