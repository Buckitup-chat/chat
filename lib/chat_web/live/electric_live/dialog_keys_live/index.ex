defmodule ChatWeb.ElectricLive.DialogKeysLive.Index do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      {:ok,
       socket
       |> Phoenix.LiveView.stream_configure(:dialog_keys, dom_id: &dom_id/1)
       |> sync_stream_fixed(:dialog_keys, DialogKey)
  ...
  @impl true
  def handle_info({:sync, {:dialog_keys, :loaded}}, socket) do
    {:noreply, assign(socket, loading: false, error: nil)}
  end

  @impl true
  def handle_info({:sync, {:dialog_keys, :live}}, socket) do
    {:noreply, assign(socket, live: true, error: nil)}
  end

  @impl true
  def handle_info({:sync, {:dialog_keys, {:error, reason}}}, socket) do
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-gray-50 py-8">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-3xl font-bold text-gray-900">
                Dialog Keys Stream (LiveView + Electric)
              </h1>
              <p class="mt-2 text-sm text-gray-600">
                Real-time dialog key exchange rows
              </p>
              <p class="mt-1 text-xs text-gray-500 font-mono">
                Using sync("/dialog_key", Chat.Data.Schemas.DialogKey) endpoint
              </p>
            </div>
        ...
        <.loading_spinner :if={@loading} label="Syncing dialog keys from Electric..." />

        <div :if={!@loading} class="bg-white shadow overflow-hidden sm:rounded-lg">
          <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Dialog Keys Stream</h3>
          </div>
