defmodule ChatWeb.ElectricLive.DialogMessagesLive.Index do
  @moduledoc "LiveView listing dialog messages (current versions) synced via Electric."

  use ChatWeb.ElectricLive.StreamIndex,
    stream: :dialog_messages,
    table: "dialog_messages",
    schema: Chat.Data.Schemas.DialogMessage

  defp dom_id(%DialogMessage{message_id: mid}), do: "dm-#{mid}"

  @impl true
  def render(assigns) do
    ~H"""
    <StreamIndex.page
      title="Dialog Messages Stream (LiveView + Electric)"
      subtitle="Real-time dialog messages (current versions)"
      stream_header="Dialog Messages Stream"
      loading_text="Syncing dialog messages from Electric..."
      loading={@loading}
      connected={@connected}
      live={@live}
    >
              ...
              <div class="flex items-center min-w-0 flex-1">
                ...
                <div class="min-w-0 flex-1 px-4">
                  ...
                  <p class="mt-1 text-sm text-gray-500 font-mono truncate">
                    dialog: {short_hex(msg.dialog_hash)} &middot; sender: {short_hex(msg.sender_hash)}
                  </p>
