defmodule ChatWeb.ElectricLive.UserSandboxLive.Index do
  ...
  def handle_event("update_name", %{"new_name" => new_name}, socket) do
    ...
    case ApiClient.update_user_name(user, user.sign_skey, new_name, base_url) do
      {:ok, %{log_entries: log_entries}} ->
        ...
        updated_user = %{user | name: new_name, owner_timestamp: new_timestamp}
  ...
  def handle_event("export_keys", _params, socket) do
    ...
    filename = "identity_#{short_hash(user.user_hash_hex)}.json"
    {:noreply, push_event(socket, "download_file", %{data: identity_json, filename: filename})}
  end
  ...
  def handle_event("import_keys", _params, socket) do
    ...
    socket =
      case parse_and_validate_identity(result) do
        {:ok, user_data} ->
          socket
          |> assign(:user, user_data)
          |> assign(:storage_items, [])
  ...
  defp parse_and_validate_identity(json_string) do
